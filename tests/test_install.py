...
def test_install_args():
    ...
    args = ["-m", "pip", "--disable-pip-version-check", "install", "dummypkg", "--requirement", "testrequirement",
            "--constraint", "constraint1", "constraint2", "--no-deps", "--pre", "--editable", "testeditable",
            "--target", "testtarget", "--platform", "testplatform", "--python-version", "3.9.0", "--implementation",
            "testimpl", "--abi", "testabi", "--user", "--root", "testroot", "--prefix", "testprefix", "--src",
            "testsrc", "--upgrade", "--upgrade-strategy", "teststrategy", "--force-reinstall", "--ignore-installed",
            "--ignore-requires-python", "--no-build-isolation", "--use-pep517", "--no-use-pep517", "--install-option",
            "\"installoption1\"", "\"installoption2\"", "--global-option", "\"globaloption1\"", "\"globaloption2\"",
            "--compile", "--no-compile", "--no-warn-script-location", "--no-warn-conflicts", "--no-binary",
            "testnobinary", "--only-binary", "testonlybinary", "--prefer-binary", "--require-hashes", "--no-clean"]
